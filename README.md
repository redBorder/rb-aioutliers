...
The anomalies are outputed in Json format together with the reconstructed data.
...
Additionaly, an user can request to perform an anomaly detection to a set of data directly. To do so, one should send a 'data' parameter instead of a 'query' parameter, this data should be a base 64 encoded JSON with a druid timeseries response.
